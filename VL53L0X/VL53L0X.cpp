...
void VL53L0X::writeReg(uint8_t reg, uint8_t value)
{
  ...
  WireX->write(reg);
  WireX->write(value);
  last_status = WireX->endTransmission();
}
...
void VL53L0X::writeReg16Bit(uint8_t reg, uint16_t value)
{
  WireX->beginTransmission(address);
  WireX->write(reg);
  WireX->write((value >> 8) & 0xFF); // value high byte
  WireX->write( value       & 0xFF); // value low byte
  last_status = WireX->endTransmission();
}
...
void VL53L0X::writeReg32Bit(uint8_t reg, uint32_t value)
{
  WireX->beginTransmission(address);
  WireX->write(reg);
  WireX->write((value >> 24) & 0xFF); // value highest byte
  WireX->write((value >> 16) & 0xFF);
  WireX->write((value >>  8) & 0xFF);
  WireX->write( value        & 0xFF); // value lowest byte
  last_status = WireX->endTransmission();
}
...
uint8_t VL53L0X::readReg(uint8_t reg)
{
  ...
  WireX->requestFrom(address, (uint8_t)1);
  value = WireX->read();
...
uint16_t VL53L0X::readReg16Bit(uint8_t reg)
{
  ...
  WireX->requestFrom(address, (uint8_t)2);
  value  = (uint16_t)WireX->read() << 8; // value high byte
  value |=           WireX->read();      // value low byte
...
uint32_t VL53L0X::readReg32Bit(uint8_t reg)
{
  ...
  WireX->requestFrom(address, (uint8_t)4);
  value  = (uint32_t)WireX->read() << 24; // value highest byte
  value |= (uint32_t)WireX->read() << 16;
  value |= (uint16_t)WireX->read() <<  8;
  value |=           WireX->read();       // value lowest byte
...
void VL53L0X::readMulti(uint8_t reg, uint8_t * dst, uint8_t count)
{
  ...
  WireX->requestFrom(address, count);
